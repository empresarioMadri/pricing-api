...
public class ApiExceptionHandler {
  ...
  public record ApiError(
    ...
    public static ApiError of(HttpStatus s, String message, String path) {
      return new ApiError(
          s.value(), s.getReasonPhrase(), message, path, OffsetDateTime.now(ZoneOffset.UTC));
    }
  }

  @ExceptionHandler(PriceNotFoundException.class)
  public ResponseEntity<ApiError> handleAppNotFound(
  ...
  public ResponseEntity<ApiError> handleUnprocessable(
    ...
    return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
        .body(
            ApiError.of(HttpStatus.UNPROCESSABLE_ENTITY, ex.getMessage(), request.getRequestURI()));
  }
